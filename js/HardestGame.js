(function () {
  ...
  //口红资源数组
  var arrayLipstick = ["../img/Lipstick-big.png", "../img/Lipstick-big.png", "../img/Lipstick-big.png"];
  //中心圆资源数组
  ...
  //子弹宽高
  var AimBullet = {w: 48 * ratio, h: 110 * ratio}
  var AimBulletBottom = CANVAS_HEIGHT - 120 * ratio - AimBullet.h + BULLET_PIERCED_DEEP;
  //调参Json
  /*var Difficulty_Json = {
      "SUCCESS_PARAMETERS" :{
  ...
  var D_Json = {
    ...
    "Level2_PARAMETERS": {
      ...
      "ROTAION_SPEED_ARRAY": [-0.05, -0.04, -0.02, 0.02, 0.04, 0.05],
      "rotationAccelerationSpeed": 0.001,
      "levelArray": [0, 8, -0.01, 40]

    },
    "Level3_PARAMETERS": {
      // "ROTAION_SPEED_ARRAY": [-0.09, -0.07, -0.05, 0.06, 0.08, 0.09],
      // "rotationAccelerationSpeed": 0.002,
      // "levelArray": [0, 12, 0.04, 60]

      "ROTAION_SPEED_ARRAY": [-0.05, -0.04, -0.02, 0.02, 0.04, 0.05],
      "rotationAccelerationSpeed": 0.001,
      "levelArray": [0, 10, -0.01, 50]
  ...
  Game.prototype = {
    ...
    normalResults: function () {
      if (this.bulletCircles[BulletCirclesIndex]) {
        var len = parseInt(this.getParams(this.levelA).levelArray[1])
        // this.bulletCircles[BulletCirclesIndex].imgObj.src = "../img/Sword_small_"+this.level+"_gray.png";
        $('#lipstick-num').text('X' + (len - (BulletCirclesIndex + 1)))
        BulletCirclesIndex++;
    ...
    nextLevel: function () {
      ...
      if (self.level <= self.levelLength) {
        ...
        // $("#levelSwitchBoxMain").attr("src", "../img/level_"+self.level+"_mains.png");
        $("#levelSwitchBoxMain").css({
          backgroundImage: 'url("../img/level_' + self.level + '_mains.jpg")'
        })
    ...
    levelChange: function () {
      ...
      for (var i = 0; i < this.levelLength; i++) {
        if (i <= this.levelA) {
          document.getElementById("levelbox").children[i].classList.add('active')
        } else {
          document.getElementById("levelbox").children[i].classList.remove('active')
        }
      }
      ...
      if (arrayCenterCircle[this.levelA]) {
        this.centerCircle.imgObj.src = arrayCenterCircle[this.levelA]
      }
